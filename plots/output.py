...
class RequestOutput:
    ...
    @classmethod
    def from_vllm_output(cls, req_id: str, req_output, modality_token_index: int = -1) -> "RequestOutput":
        aborted = (
...
class ExperimentOutput:
    ...
    def save_engine_stats(self, log_file: Union[str,LiteralString]):
        path = os.path.join(self.engine_stats_path, f"{self.id}.parquet")
    ...
    def save(self):
        path = os.path.join(self.output_path, f"{self.id}.jsonl")
        ...
        with open(self.output_log_path, "a", encoding="utf-8") as file:
